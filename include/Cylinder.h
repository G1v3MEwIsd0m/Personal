...
public:
   ...
   /* Cylinder(float baseRadius=1.0f, float topRadius=1.0f, float height=1.0f,
             int sectorCount=36, int stackCount=1, bool smooth=true, int up=3);*///change smooth to color
    Cylinder(float baseRadius = 1.0f, float topRadius = 1.0f, float height = 1.0f,
        int sectorCount = 36, int stackCount = 1, std::vector<float>vertColors = { 0.2, 0.4, 0.5 } , int up = 3);
    ~Cylinder() {}
    ...
    void set(float baseRadius, float topRadius, float height,
             int sectorCount, int stackCount, std::vector<float>vertColors, int up=3);
    ...
    const std::vector<Vertex> getCylVertices() const { return cylVertices;}
    const std::vector<uint32_t> getCylIndices() const { return cylIndices;}
    //const float* getVertices() const { return vertices.data(); }
    //const unsigned int* getIndices() const { return indices.data(); }
