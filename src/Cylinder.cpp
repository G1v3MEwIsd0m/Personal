...
//                   int stacks, bool smooth, int up) : interleavedStride(32)//Change smooth to colors
//{
...
Cylinder::Cylinder(float baseRadius, float topRadius, float height, int sectors, int stacks, std::vector<float>vertColors, int up)
{

    set(baseRadius, topRadius, height, sectors, stacks, vertColors, up);
}
...
void Cylinder::set(float baseRadius, float topRadius, float height, int sectors,
                   int stacks, std::vector<float>vertColors, int up)
{
    if(baseRadius > 0)
...
void Cylinder::setBaseRadius(float radius)
{
    if(this->baseRadius != radius)
        set(radius, topRadius, height, sectorCount, stackCount, vertColors, upAxis);
}

void Cylinder::setTopRadius(float radius)
{
    if(this->topRadius != radius)
        set(baseRadius, radius, height, sectorCount, stackCount, vertColors, upAxis);
}

void Cylinder::setHeight(float height)
{
    if(this->height != height)
        set(baseRadius, topRadius, height, sectorCount, stackCount, vertColors, upAxis);
}

void Cylinder::setSectorCount(int sectors)
{
    if(this->sectorCount != sectors)
        set(baseRadius, topRadius, height, sectors, stackCount, vertColors, upAxis);
}

void Cylinder::setStackCount(int stacks)
{
    if(this->stackCount != stacks)
        set(baseRadius, topRadius, height, sectorCount, stacks, vertColors, upAxis);
}
...
void Cylinder::allTogether() {

    int sizeVerts = vertices.size();
    int sizetexCoords = texCoords.size();
    int sizeNormals = normals.size();
    /*std::cout << "Verts: "<< sizeVerts << std::endl;
    std::cout << "Tex: " << sizetexCoords << std::endl;
    std::cout << "Normals: " << sizeNormals << std::endl;*/
    Vertex tempVertex = {
        .Position = { vertices[sizeVerts - 3], vertices[sizeVerts - 2], vertices[sizeVerts - 1]},
        .Color = {vertColors[0],vertColors[1],vertColors[2]},
        .Normal = {normals[sizeNormals - 3], normals[sizeNormals - 2],normals[sizeNormals - 1] },
        .Uv = {texCoords[sizetexCoords - 2], texCoords[sizetexCoords - 1]}
    };
    cylVertices.emplace_back(tempVertex);
}
...
{
    cylIndices.emplace_back(i1);
    cylIndices.emplace_back(i2);
    cylIndices.emplace_back(i3);
